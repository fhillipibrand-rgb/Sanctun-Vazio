-- 1. Primeiro, precisamos adicionar a coluna days_of_week que estava faltando na tabela
ALTER TABLE public.workout_routines 
ADD COLUMN IF NOT EXISTS days_of_week JSONB DEFAULT '[]'::jsonb;

-- 2. Agora o bloco de inserção da rotina
DO $$ 
DECLARE
    v_user_id UUID;
    v_program_id UUID;
...
BEGIN
    ...
    VALUES (v_user_id, 'Hipertrofia 5x na Semana', 'Divisão ABCDE focada em ganho máximo de massa muscular (Seg a Sex).', true)
    RETURNING id INTO v_program_id;

    -- Inserir Rotina A (Segunda - Peito)
    INSERT INTO public.workout_routines (user_id, program_id, name, description, days_of_week)
    VALUES (v_user_id, v_program_id, 'Treino A - Peito', 'Foco na porção maior do peitoral e expansão', '["Seg"]')
    ...
    -- Inserir Rotina B (Terça - Costas)
    INSERT INTO public.workout_routines (user_id, program_id, name, description, days_of_week)
    VALUES (v_user_id, v_program_id, 'Treino B - Costas', 'Desenvolvimento de dorsais e espessura', '["Ter"]')
    ...
    -- Inserir Rotina C (Quarta - Pernas)
    INSERT INTO public.workout_routines (user_id, program_id, name, description, days_of_week)
    VALUES (v_user_id, v_program_id, 'Treino C - Pernas', 'Treino completo de membros inferiores', '["Qua"]')
    ...
    -- Inserir Rotina D (Quinta - Ombros e Abdômen)
    INSERT INTO public.workout_routines (user_id, program_id, name, description, days_of_week)
    VALUES (v_user_id, v_program_id, 'Treino D - Ombros e Core', 'Volume para deltoides e fortalecimento do core', '["Qui"]')
    ...
    -- Inserir Rotina E (Sexta - Braços)
    INSERT INTO public.workout_routines (user_id, program_id, name, description, days_of_week)
    VALUES (v_user_id, v_program_id, 'Treino E - Braços', 'Foco exclusivo em bíceps e tríceps', '["Sex"]')
