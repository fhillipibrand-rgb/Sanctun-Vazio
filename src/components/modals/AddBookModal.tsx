...
// Configuração otimizada para o Worker
...
const AddBookModal: React.FC<AddBookModalProps> = ({ isOpen, onClose, onSave, bookToEdit }) => {
  ...
  const countPdfPages = async (file: File): Promise<number> => {
    return new Promise(async (resolve) => {
      // Timeout de 5 segundos para não travar o formulário
      const timeout = setTimeout(() => {
        console.warn("Timeout ao contar páginas do PDF.");
        resolve(0);
      }, 5000);
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      if (bookToEdit) {
        const { error } = await supabase.from('books').update(formData).eq('id', bookToEdit.id);
        if (error) throw error;
      } else {
        const { error } = await supabase.from('books').insert({ ...formData, user_id: user.id, current_page: 0 });
        if (error) throw error;
      }
      if (onSave) onSave();
      onClose();
        ...
        <GlassCard className="p-0 border-secondary/30 shadow-2xl">
          ...
          <form onSubmit={handleSave} className="p-8 space-y-6">
            <div className="space-y-4">
              ...
              <div className="space-y-1.5">
                <label className="text-[9px] font-bold opacity-40 uppercase tracking-widest flex items-center gap-2"><ImageIcon size={10} /> CAPA</label>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                   <div className="space-y-2">
                      ...
                      <button type="button" onClick={() => imageInputRef.current?.click()} disabled={uploadingImage} className="w-full aspect-video flex flex-col items-center justify-center gap-2 border-2 border-dashed border-on-surface/10 hover:border-secondary/40 bg-on-surface/5 rounded-2xl transition-all group overflow-hidden relative">
                        {imagePreview ? <img src={imagePreview} className="w-full h-full object-cover" alt="Preview" /> : uploadingImage ? <Loader2 className="animate-spin text-secondary" /> : <><Upload size={20} className="opacity-40 group-hover:text-secondary group-hover:opacity-100 transition-all" /><p className="text-[10px] font-bold opacity-40 uppercase">Upload Imagem</p></>}
                      </button>
                   </div>
                   <div className="flex flex-col justify-center">
                      <input type="url" value={formData.cover_url} onChange={e => setFormData({...formData, cover_url: e.target.value})} className="w-full bg-on-surface/5 border border-[var(--glass-border)] rounded-xl px-4 py-3 text-[10px] font-bold outline-none focus:border-secondary/50 transition-colors" placeholder="Link da imagem..." />
                   </div>
                </div>
              </div>
                ...
                <div className="space-y-2">
                  ...
                  <button type="button" onClick={() => pdfInputRef.current?.click()} disabled={uploadingPdf} className={`w-full flex items-center gap-4 p-4 border-2 border-dashed rounded-2xl transition-all ${pdfFileName ? 'border-secondary/40 bg-secondary/5' : 'border-on-surface/10 bg-on-surface/5 hover:border-secondary/40'}`}>
                    ...
                    <div className="flex-1 text-left min-w-0">
                      ...
                      <p className="text-[11px] font-bold truncate opacity-60">{pdfFileName || 'Máximo 50MB'}</p>
                    </div>
