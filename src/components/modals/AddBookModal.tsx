...
import { X, Save, Book, User, Hash, ImageIcon, FileText, Loader2 } from "lucide-react";
          ...
          <form onSubmit={handleSave} className="p-8 space-y-6">
            ...
            <div className="pt-6 border-t border-[var(--glass-border)] flex justify-end gap-3">
              <button 
                type="button"
                onClick={onClose} 
                className="px-6 py-2 rounded-xl border border-[var(--glass-border)] text-[10px] font-bold uppercase tracking-widest"
              >
                CANCELAR
              </button>
              <button 
                type="submit"
                disabled={loading}
                className="px-8 py-2 rounded-xl bg-secondary text-surface text-[10px] font-bold tracking-widest uppercase flex items-center gap-2 hover:scale-105 transition-all shadow-lg shadow-secondary/20 disabled:opacity-50"
              >
