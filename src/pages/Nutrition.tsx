...
const Nutrition = () => {
  ...
  const [water, setWater] = useState({ current: 0, target: 12 });
  const [meals, setMeals] = useState({ breakfast: false, lunch: false, afternoon: false, dinner: false });
  const [supps, setSupps] = useState([
    { name: "Creatina", time: "Manhã", done: false },
    { name: "Multivitamínico", time: "Almoço", done: false },
    { name: "Ômega 3", time: "Jantar", done: false }
  ]);
  ...
  const updateWater = async (newAmount: number) => {
    ...
    if (!error) {
      setWater(prev => ({ ...prev, current: newAmount }));
    }
  };
  ...
  return (
    <div className="space-y-8 max-w-6xl mx-auto pb-20 pt-4">
      ...
      {loading ? (
        <div className="flex items-center justify-center py-20 opacity-30 editorial-label animate-pulse">SINCRONIZANDO METAS...</div>
      ) : (
          ...
          <GlassCard className="p-8 border-cyan-400/20 relative overflow-hidden group col-span-1 lg:col-span-1">
            ...
            <div className="relative z-10 flex flex-col h-full">
              ...
              <div className="flex gap-4 mt-auto">
                <button 
                  onClick={() => updateWater(Math.max(0, water.current - 1))}
                ...
                <button 
                  onClick={() => updateWater(water.current + 1)}
